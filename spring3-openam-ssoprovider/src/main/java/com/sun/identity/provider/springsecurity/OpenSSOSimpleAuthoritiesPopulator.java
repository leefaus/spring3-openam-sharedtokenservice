 ...
 */
public class OpenSSOSimpleAuthoritiesPopulator implements OpenSSOGrantedAuthoritiesMapper {

    /**
     * Lookup the users group memberships and return as a collection of GrantedAuthority
     * @param ssoToken users SSOTOken
     * @return Collection of GrantedAuthority representing the user's OpenSSO groups.
     * 
     * @throws com.sun.identity.idm.IdRepoException
     * @throws com.iplanet.sso.SSOException
     */
    public Collection<GrantedAuthority> getGrantedAuthorities(SSOToken ssoToken) throws IdRepoException, SSOException {
        ...
        if (groups != null && groups.size() > 0) {
            ...
            ga = new ArrayList<GrantedAuthority>(groups.size() + 1);
            for (Iterator<AMIdentity> itr = groups.iterator(); itr.hasNext();) {
                AMIdentity group = itr.next();
                String role = "ROLE_" + group.getName().toUpperCase();
                ga.add(new GrantedAuthorityImpl(role));
            }
        } else {
            ga = new ArrayList<GrantedAuthority>(1);
        }
